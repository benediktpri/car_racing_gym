...
from gymnasium.wrappers import GrayScaleObservation, ResizeObservation, FrameStack, ClipAction
import math
import random
import matplotlib
...
class DQN(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(64 * 7 * 7, 512)  # Size needs to be adjusted depending on the output from the last conv layer
        self.fc2 = nn.Linear(512, 5)  # Output layer: corresponds to the number of actions
    ...
    def forward(self, x):
        ...
        # Output layer (no activation needed here, as this is not a classification problem but regression-like)
        x = self.fc2(x)
        return x
